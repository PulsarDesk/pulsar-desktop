...
/// `stat …` feeds the HUD, `pace 0|1` toggles frame pacing.
fn stdin_control() {
    use std::io::BufRead;
    ...
    unsafe fn paint_overlay(&mut self) {
        ...
        let raw = egui::RawInput {
            screen_rect: Some(egui::Rect::from_min_size(
                egui::pos2(0.0, 0.0),
                egui::vec2(self.width as f32 / OVERLAY_PPP, self.height as f32 / OVERLAY_PPP),
            )),
            events,
            ..Default::default()
        };
        ...
        let out = self.egui_ctx.run(raw, |ctx| {
            cmds = crate::overlay::draw(ctx, ostate);
        });
    ...
    unsafe fn show_frame(&mut self, nv12: &ID3D11Texture2D) {
        ...
        if self.present.is_none() || sw != self.src_w || sh != self.src_h {
            ...
            match self.present.as_mut() {
                ...
                None => match present::Present::new(&self.device, sw, sh, self.width, self.height) {
        ...
        let Some(back) = self.swap.GetBuffer::<ID3D11Texture2D>(0).ok() else { return };
