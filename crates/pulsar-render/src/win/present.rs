...
use windows::Win32::Graphics::Direct3D11::{
    ...
    D3D11_VPOV_DIMENSION_TEXTURE2D, D3D11_TEX2D_VPIV, D3D11_TEX2D_VPOV,
};
...
impl Present {
    pub unsafe fn new(device: &ID3D11Device, in_w: u32, in_h: u32, out_w: u32, out_h: u32) -> Result<Self> {
        ...
        Ok(Self { vdevice, vctx, vp_enum, vproc, in_w, in_h, out_w, out_h })
    }
    ...
    fn dest_rect(&self) -> RECT {
        let (iw, ih, ow, oh) = (self.in_w as f32, self.in_h as f32, self.out_w as f32, self.out_h as f32);
        if iw <= 0.0 || ih <= 0.0 {
            return RECT { left: 0, top: 0, right: self.out_w as i32, bottom: self.out_h as i32 };
        }
    ...
    pub unsafe fn blt(&self, src: &ID3D11Texture2D, back: &ID3D11Texture2D) -> Result<()> {
        ...
        self.vdevice
            .CreateVideoProcessorOutputView(back, &self.vp_enum, &out_desc, Some(&mut ov))?;
        let ov = ov.unwrap();

        // Letterbox: clear to background + place the video in the aspect-fit dest rect.
        let dst = self.dest_rect();
        self.vctx.VideoProcessorSetStreamDestRect(&self.vproc, 0, true, Some(&dst));
