...
impl CaptureDevice {
    ...
    pub(super) unsafe fn build_duplication(&mut self) -> windows::core::Result<()> {
        ...
        for attempt in 0..3 {
            ...
            match self.output.DuplicateOutput(&self.device) {
                ...
                Err(e) => {
                    ...
                    {
                        if let Ok((_, out1)) =
                            Self::find_output(&self.factory, self.output_idx)
                        {
                            self.output = out1;
                        }
                    }
        ...
        Err(last_err.unwrap_or_else(|| {
            windows::core::Error::from(DXGI_ERROR_NOT_CURRENTLY_AVAILABLE)
        }))
    }
    ...
    pub fn target_size(&self, want_w: u32, want_h: u32) -> (u32, u32) {
        ...
        } else if std::env::var("PULSAR_CAP_1080").map(|v| v == "1").unwrap_or(false) && nh > 1080 {
