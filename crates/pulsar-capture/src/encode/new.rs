...
impl Encoder {
    ...
    pub unsafe fn new(
        ...
        let cap_w = if p.capture_width >= 2 { p.capture_width & !1 } else { w };
        let cap_h = if p.capture_height >= 2 { p.capture_height & !1 } else { h };
        ...
        let (nv_device, nv_context, bridge): (
            ...
            let (nv_dev, nv_ctx) = create_nvidia_device()
                .map_err(|e| format!("hybrid: create NVIDIA device: {e}"))?;
        ...
        let bitrate_kbps =
            std::sync::Arc::new(std::sync::atomic::AtomicU32::new(p.bitrate_kbps));
        ...
        let mut fns: Box<nvenc::NV_ENCODE_API_FUNCTION_LIST> =
        ...
        let idr_sec = std::env::var("PULSAR_IDR_SEC").ok().and_then(|s| s.parse::<u32>().ok()).filter(|&v| v > 0).unwrap_or(4);
        ...
        match p.codec {
            ...
            Codec::Av1 => {
                ...
                av1.set_flags(
                    nvenc::NV_ENC_AV1_FLAG_REPEAT_SEQ_HDR | nvenc::NV_ENC_AV1_CHROMA_420,
                );
                av1.idrPeriod = gop;
        ...
        chk(reg_fn(enc, &mut reg), &fns, enc)
        ...
        let (amd_shared, amd_context, nvidia_bgra, kept_amd_device, kept_amd_context) =
            match bridge {
                Some(b) => (
                    Some(b.amd_shared),
                    Some(b.amd_context),
