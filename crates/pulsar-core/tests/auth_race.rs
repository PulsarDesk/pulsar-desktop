...
async fn spawn_host(host: Arc<Node>, allow: oneshot::Receiver<bool>, accepted: Arc<AtomicBool>) {
	tokio::spawn(async move {
		let Some(mut session) = host.next_incoming().await else { return };
...
async fn fresh() -> (SocketAddr, Arc<Node>, Arc<Node>) {
	...
	let host = Node::bind(LOCAL.parse().unwrap(), addr, NetworkMode::Auto).await.unwrap();
	let client = Node::bind(LOCAL.parse().unwrap(), addr, NetworkMode::Auto).await.unwrap();
...
async fn host_approves_without_password() {
	...
	assert!(matches!(recv_host_auth(&mut sess).await, HostAuth::NeedPassword));

	// Host clicks "Allow" — no password ever entered.
	allow_tx.send(true).unwrap();
	let v = timeout(Duration::from_secs(2), recv_host_auth(&mut sess)).await.unwrap();
	assert!(matches!(v, HostAuth::Ok), "host Allow must accept passwordlessly");
}
