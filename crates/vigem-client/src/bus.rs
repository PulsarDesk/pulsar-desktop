...
// DS4 rumble/LED notification: the driver completes this pending IOCTL when the consuming
// app sends an output report (rumble/lightbar), writing the motor + lightbar values back.
// IOCTL_BASE(0x801) + 0x203 with FILE_WRITE_DATA access (BUSENUM_W_IOCTL, per ViGEm
// BusShared.h) → 0x2AA810. NOTE: unlike the XUSB notification (RW, 0x2AE804), the DS4 one
// is WRITE access; using RW (0x2AE810) makes the IOCTL fail immediately.
pub const IOCTL_DS4_REQUEST_NOTIFICATION: u32 = 0x2AA810;
// NOTE: there is NO separate "DS4 submit report EX" IOCTL — the EXTENDED 63-byte report
// goes through IOCTL_DS4_SUBMIT_REPORT above and ViGEmBus tells basic/EX apart by buffer
// SIZE (DS4_SUBMIT_REPORT_EX is larger). See DS4SubmitReportEx below.
...
// PACKED: ViGEm's DS4_SUBMIT_REPORT_EX lives under <pshpack1.h> (like DS4_REPORT_EX), so
// it's 71 bytes (4+4+63) with NO trailing alignment padding. A plain `#[repr(C)]` here
// pads to 72 and the driver rejects it with STATUS_INVALID_BUFFER_SIZE (ERROR 1784),
// because it checks `Size == sizeof(DS4_SUBMIT_REPORT_EX)` == 71. (The legacy basic
// DS4SubmitReport is naturally aligned — only the EX structs are pshpack1.)
#[cfg(feature = "unstable_ds4")]
#[repr(C, packed)]
...
#[cfg(feature = "unstable_ds4")]
const _: () = assert!(mem::size_of::<DS4SubmitReportEx>() == 71);

/// In/out buffer for IOCTL_DS4_REQUEST_NOTIFICATION. We submit it (Size + SerialNo set);
/// the driver fills the motors + lightbar when the app sends an output report. Field order
/// MUST match ViGEm's `DS4_OUTPUT_REPORT`: SmallMotor FIRST, then LargeMotor, then the
/// lightbar RGB (not packed — natural alignment, total 16 bytes).
#[cfg(feature = "unstable_ds4")]
#[repr(C)]
pub struct Ds4AwaitNotification {
	pub Size: u32,
	pub SerialNo: u32,
	pub SmallMotor: u8,
	pub LargeMotor: u8,
...
#[cfg(feature = "unstable_ds4")]
impl Ds4AwaitNotification {
	#[inline]
	pub const fn new(serial_no: u32) -> Ds4AwaitNotification {
		Ds4AwaitNotification {
			Size: mem::size_of::<Ds4AwaitNotification>() as u32,
			SerialNo: serial_no,
			SmallMotor: 0,
			LargeMotor: 0,
			LightbarR: 0,
	...
	/// Blocks until the consuming app sends an output report (rumble/LED) or the target is
	/// unplugged (then GetOverlappedResult fails — caller treats it as "stop"). In and out
	/// buffer are the same struct (the driver writes the motor values back into it).
	#[inline]
	pub unsafe fn ioctl(&mut self, device: HANDLE, event: HANDLE) -> Result<(), u32> {
		let mut transferred = 0;
		let mut overlapped: OVERLAPPED = mem::zeroed();
		overlapped.hEvent = event;
		let buf = self as *mut _ as _;
		let sz = mem::size_of_val(self) as u32;
		DeviceIoControl(
			device,
...
impl DS4SubmitReportEx {
	...
	pub unsafe fn ioctl(&mut self, device: HANDLE, event: HANDLE) -> Result<(), u32> {
		...
		// IMPORTANT: the extended report is submitted via the SAME IOCTL as the basic
		// report — ViGEmBus distinguishes basic vs EX purely by the input buffer SIZE
		// (DS4_SUBMIT_REPORT_EX is larger). There is NO separate "EX" IOCTL function code;
		// using one makes the driver return ERROR_NOT_SUPPORTED. (See ViGEmClient
		// vigem_target_ds4_update_ex: "Same IOCTL, just different size".)
		DeviceIoControl(
			device,
			IOCTL_DS4_SUBMIT_REPORT,
			self as *mut _ as _,
